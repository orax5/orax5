...
contract DtsToken is ERC1155, Ownable{
    ...
    // 거버넌스 투표 기간
    mapping (uint256 => uint256) public votingDate;
    ...
    // 거버넌스 신청 함수
    function startVoting(uint256 tokenId, uint256 date) public{
        ...
        // 거버넌스 투표는 한번만 가능
        require(votingDate[tokenId] != 0, "There is already a vote.");
        votingDate[tokenId] = block.timestamp * date;
    }
    ...
    function isVoting(uint256 tokenId, bool governance) public{
        ...
        require(ERC1155.balanceOf(msg.sender, tokenId) != 0, "no amount");
        // 거버넌스 투표 기간이 지났는지 확인
        require(block.timestamp <= votingDate[tokenId], "Time Out" );
    ...
    function endVoting(uint256 tokenId, bool result) public{
        // 거버넌스 투표 기간이 지났는지 확인 
        require(votingDate[tokenId] < block.timestamp,"Time Out?");
        // nft 보유하고 있는지?
        require(ERC1155.balanceOf(msg.sender, tokenId) != 0, "no amount");
        // 펀딩이 이미 끝났는지
        ...
        if(result == true){
        tokenOwners[tokenId].EndTime = tokenOwners[tokenId].EndTime + 86400 * 14;
        }
    }
